...
- 上传前检查同名文件，按素材类型统计，并支持覆盖原文件或自动重命名
...
- 矩形框和多边形标注叠加预览
- 图片与标注自动关联、模型与数据集版本关联、关系历史和模型溯源
- 素材软删除、回收站还原和后台物理清理
- 统一任务中心：类型、状态与创建时间筛选、失败重试、导出取消、任务心跳和限时下载
- 64 MB 以上文件分片上传，支持暂停、继续和重新选择同一文件后恢复
- 用户、角色、标签、格式和审计数据管理；默认标签受删除保护
